...
export function BlogPage() {
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'Blog', name: 'Vergeform Insights', description: 'Practical insights on AI, software development, cybersecurity, and business automation.', url: `${agency.siteUrl}/blog`, publisher: { '@type': 'Organization', name: agency.name } }

  return <>
    <Seo title="AI, Development & Automation Insights — Vergeform" description="Practical guides on AI agents, AI-native software development, cybersecurity, full-stack engineering, and business automation." path="/blog" keywords={['AI business blog', 'AI software development', 'AI automation', 'cybersecurity insights']} schema={schema} />
    ...
    <div className="blog-index" id="latest">
      {featuredPost && <section className="blog-feature container" aria-labelledby="featured-title"><Reveal className="blog-feature__shell">
        <Link className="blog-feature__visual" to={`/blog/${featuredPost.slug}`} aria-label={`Read ${featuredPost.title}`}><BlogVisual variant={featuredPost.visual} /><span className="blog-feature__edition">01 <small>/ Editor’s pick</small></span></Link>
      ...
      <section className="section blog-library" aria-labelledby="library-title"><div className="container">
        ...
        <div className="blog-grid">{articles.map((post, index) => <Reveal className="blog-card" delay={Math.min(index * 0.04, 0.12)} key={post.slug}><Link className="blog-card__visual" to={`/blog/${post.slug}`} aria-label={`Read ${post.title}`}><BlogVisual variant={post.visual} /><span>{post.category}</span><b className="blog-card__number">{String((featuredPost ? index + 2 : index + 1)).padStart(2, '0')}</b></Link><div className="blog-card__body"><ArticleMeta post={post} /><h3><Link to={`/blog/${post.slug}`}>{post.title}</Link></h3><p>{post.excerpt}</p><Link className="blog-card__link" to={`/blog/${post.slug}`}>Read the guide <ArrowUpRight aria-hidden="true" /></Link></div></Reveal>)}</div>
