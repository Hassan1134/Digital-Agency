...
import { packages, serviceGroups } from '../data/siteContent'
import { ProjectPlanner } from '../components/modules/ProjectPlanner'
...
export function ServicesPage() {
  ...
  return <>
    ...
    <section className="section service-groups"><div className="container"><div className="section-heading"><p className="eyebrow">Capability map</p><h2>Everything connected.<br /><em>Nothing buried.</em></h2><p>Start with a discipline or combine the capabilities around a larger business outcome.</p></div><div className="service-groups__grid">{serviceGroups.map((group, index) => <article key={group.id}><span>{String(index + 1).padStart(2, '0')}</span><h3>{group.title}</h3><p>{group.summary}</p><div>{group.serviceIds.map((id) => { const service = services.find((item) => item.id === id); return service && <a href={`#${service.id}`} key={id}>{service.short} <ArrowRight aria-hidden="true" /></a> })}</div></article>)}</div></div></section>
    <section className="section package-section surface-dark"><div className="container"><div className="section-heading section-heading--light"><p className="eyebrow">Starting shapes</p><h2>Common needs,<br />clearly framed.</h2><p>These are useful starting points, not rigid bundles. Final scope, timing, and investment follow a short discovery.</p></div><div className="package-grid">{packages.map((item, index) => <article className="package-card" key={item.title}><span>{String(index + 1).padStart(2, '0')}</span><p className="mini-label">{item.ideal}</p><h3>{item.title}</h3><p>{item.duration}</p><strong>{item.investment}</strong><ul>{item.includes.map((line) => <li key={line}><Check aria-hidden="true" />{line}</li>)}</ul><Button to={`/contact?service=${item.serviceId}`}>Request a tailored scope</Button></article>)}</div></div></section>
    <ProjectPlanner />
